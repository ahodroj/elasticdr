...
if ( warUrl ) {
	new AntBuilder().sequential {
		if ( warUrl.toLowerCase().startsWith("http") || warUrl.toLowerCase().startsWith("ftp")) {
			...
			echo(message:"Copying ${context.serviceDirectory}/war/petclinic.war to ${applicationWar} ...")
			copy(tofile: "${applicationWar}", file:"${context.serviceDirectory}/war/petclinic.war", overwrite:true)
		}
	}
}
